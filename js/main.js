...
function initDropdownAnimation() {
    ...
    if (!dropdownWrapper || !menu || items.length === 0) return;

    // Create a GSAP timeline that is paused by default
    const tl = gsap.timeline({ paused: true });

    // Step A: Show the menu box and slide it up slightly
    tl.fromTo(menu, 
    ...
    // Step B: Stagger the inner items sliding up and fading in
    tl.fromTo(items, 
        { y: 15, opacity: 0 },
        { 
            y: 0, 
    ...
    // Play on hover, reverse rapidly on leave
...
function initMockupTilt() {
    const mockups = document.querySelectorAll('.feature-visual .browser-mockup');

    mockups.forEach(mockup => {
        const wrapper = mockup.closest('.feature-visual');

        wrapper.addEventListener('mousemove', (e) => {
            const rect = mockup.getBoundingClientRect();
            // Normalize mouse position from -0.5 to 0.5 relative to the element
            const x = (e.clientX - rect.left) / rect.width - 0.5;
            const y = (e.clientY - rect.top) / rect.height - 0.5;
...
function initDashboardTilt() {
    const wrapper = document.getElementById('dbPreviewWrapper');
    const mockup = wrapper ? wrapper.querySelector('.db-browser-mockup') : null;
